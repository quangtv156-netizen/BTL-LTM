...
Tạo file `db.properties` ở thư mục gốc project (ngang hàng với `src`, `lib`) — file này **không** commit lên Git:
```properties
db.url=jdbc:mysql://localhost:3306/barricade_game?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true
db.user=root
db.password=MAT_KHAU_MYSQL_CUA_BAN
```

### 3. Cấu hình JavaFX cho Client
...
- Mật khẩu người dùng hiện lưu dạng plain text trong database — chỉ phù hợp cho mục đích học tập/đồ án, không dùng cho môi trường thật.
- File `db.properties` chứa thông tin đăng nhập MySQL, mỗi máy chạy cần tự tạo file này (không đồng bộ qua Git).
